...
Special events are a way of adding non-supported or custom events. For example, the @mouseenter@ and @mouseleave@ events are IE proprietary. Therefore, in order to make the cross-browser compatible, they were implemented as special events. The @hashchange@ event is als0 handled with specialEvents. The specific function definitions for dealing with special events are above in the Functions section.
